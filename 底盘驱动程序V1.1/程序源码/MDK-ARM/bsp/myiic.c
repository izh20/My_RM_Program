...
#if 1
#define SDA_PIN_IDX 4 //PIN_IDX X = GPIO_PIN_X    2018.10.14由11改为4
#if defined(__STM32F4xx_HAL_H)
#define SDA_IN() /*SDA(PA4)设置为输入*/                                     \
    {                                                 \
        SDA_PORT->MODER &= ~(3 << (SDA_PIN_IDX * 2)); \
        SDA_PORT->MODER |= (0 << (SDA_PIN_IDX * 2));  \
    } //as input

#define SDA_OUT()/*SDA(PA4)设置为输出*/                                     \
    {                                                 \
        SDA_PORT->MODER &= ~(3 << (SDA_PIN_IDX * 2)); \
...
//void                    SDA_OUT()                                                                             //或许多余
//{
//    GPIO_InitStruct.Pin  = SDA_PIN;                                          //端口配置   2018.10.14明天可尝试改为SDA_PIN|SCL_PIN
//    GPIO_InitStruct.Mode = GPIO_MODE_OUT_PP;//推挽输出
//    GPIO_InitStruct.Pull = GPIO_PULLUP;
//    HAL_GPIO_Init(SDA_PORT, &GPIO_InitStruct);//A口初始化
//}
...
void IIC_Init(void)
{
		GPIO_InitTypeDef GPIO_InitStruct;					 //定义结构体		 2018.10.14添加
	
	  __HAL_RCC_GPIOA_CLK_ENABLE();   					 //使能外设GPIOA时钟  2018.10.14添加
    ...
    //SDA		can change as INPUT or PUSH PULL
		...
		GPIO_InitStruct.Pin  = SDA_PIN|SCL_PIN;    //端口配置   2018.10.14改为SDA_PIN|SCL_PIN
		GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;//推挽输出     之前写为GPIO_MODE_OUT_PP，与设定不符
    GPIO_InitStruct.Pull = GPIO_PULLUP;        //上拉
		GPIO_InitStruct.Speed= GPIO_SPEED_FAST;    //快速       2018.10.14添加
		HAL_GPIO_Init(SDA_PORT, &GPIO_InitStruct); //A口初始化
	
    //SDA_OUT();   重复引用
    IIC_SCL_1;//PA5,PA4输出高
    IIC_SDA_1;
}
//产生IIC起始信号
void IIC_Start(void)
...
u8 IIC_Wait_Ack(void)
{
    ...
    IIC_SCL_1;
    IIC_Delay();
    while (READ_SDA()) //if sda was pull 0	means ACK ok!  读取GPIO4输入
    {
        ucErrTime++;
        if (ucErrTime > 250)
        {
...
//读1个字节，ack=1时，发送ACK，ack=0，发送nACK////                    看到了这里
