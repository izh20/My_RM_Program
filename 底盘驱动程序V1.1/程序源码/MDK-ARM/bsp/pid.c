...
//	uint32_t time,lasttime;
  ...
  //	pid->target = target;
	...
	pid->last_err  = pid->err;   							 //e(t-1)=e(t)
	pid->last_output = pid->output;						 //
	
	pid->err = pid->target - pid->measure;     //e(t)=目标(pwm输出数据)-测量(pwm输出数据)
	
	//是否进入死区
	if((ABS(pid->err) > pid->DeadBand))
	{
		pid->pout = pid->kp * pid->err;					 //kp*e(t)
		pid->iout += (pid->ki * pid->err);			 //
		

		pid->dout =  pid->kd * (pid->err - pid->last_err); //kd*[e(t)-e(t-1)]
		...
		//pid->output = pid->output*0.7f + pid->last_output*0.3f;  //限幅
